...
export class PostEngagementController {
    ...
    public async dislikePost(req: Request, res: Response): Promise<void> {
        // Implementation for disliking a post
        let validatedInput;
        try {
            validatedInput = CreateDisLikeSchema.parse(req.body);
        } catch (error) {
            logger.error('Invalid dislike create input!');
            res.status(400).json({ status: false, message: 'Invalid dislike create input!' });
            return;
        }

        try {
            await this.service.dislikePost(validatedInput);
            res.status(200).json({ status: true, message: 'DisLike updated successfully' });
        } catch (error) {
            if (error instanceof AppError) {
                logger.error(`${error.at}: ${error.message}`);
                res.status(error.statusCode).json({ message: error.message, status: false });
