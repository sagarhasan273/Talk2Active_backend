...
export class PostController {
    ...
    public async updatePost(req: Request, res: Response): Promise<void> {
        ...
        try {
            await this.service.updatePost(validatedInput);
            ...
            logger.error('An error occurred while updating post!');
            res.status(500).json({ message: 'An error occurred while updating post!', status: false });
        }
    }
    ...
    public async deletePost(req: Request, res: Response): Promise<void> {
        let validatedInput;
        try {
            validatedInput = DeletePostSchema.parse(req.body);
        } catch (error) {
            logger.error('Invalid post delete data!');
            res.status(400).json({ status: false, message: 'Invalid post delete data!' });
            return;
        }

        try {
            await this.service.deletePost(validatedInput);
            ...
            logger.error('An error occurred while deleting post!');
            res.status(500).json({ message: 'An error occurred while deleting post!', status: false });
        }
    }
