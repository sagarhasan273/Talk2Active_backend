...
export class UserController {
  ...
  public async logInUser(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!validatedInput) {
        throw new AppError('Email and password are required', 400, 'User Service');
      }
  ...
  public async createUser(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!validatedData) {
        throw new AppError('Invalid user data!', 400, 'User Service');
      }

      const user = await this.service.createUser(validatedData);
      if (!user) {
        throw new AppError('User did not appeared in controller!', 409, 'User Service');
      }
